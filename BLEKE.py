        ...
        for i in range(winreg.QueryInfoKey(controllers_reg)[0]):   #Controllers LOOP     
            ...
            for j in range(winreg.QueryInfoKey(devices_reg)[0]):   #Devices LOOP      
                ...
                if 'LTK' and 'CSRKInbound' in keys:
                    ...
                    + "\nAuthenticated=3\nEncSize=16\nEDiv=0\nRand=0")
                    # + "\n\n[LinkKey]"
                    # + "\nKey=" + winreg.EnumValue(devices_reg,j)[1].hex().upper() #Extracting the LinkKey. it needs to add an if
                    # + "\nType=8\nPINLength=0")                       
                             
                for k in range(1,winreg.QueryInfoKey(devices_reg)[1]): #loop to check if device also has a LinkKey
                    if (winreg.EnumValue(devices_reg,k)[0]==device):
                        #print("Hay LINKKEY k=",k, "reg=",winreg.EnumValue(devices_reg,k), "LK=", winreg.QueryValueEx(devices_reg, device)[0])
                        file_content += ("\n\n[LinkKey]"
                        + "\nKey=" + winreg.QueryValueEx(devices_reg, device)[0].hex().upper() #Extracting the LinkKey. it needs to add an if
                        + "\nType=8\nPINLength=0")   
